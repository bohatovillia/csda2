...
void handle_get_status(http_server_t srv, http_request_t req, void *user_data) {
  char response[64];
  snprintf(response, sizeof(response), "{\"led_on\": %s}",
 ...
 *---------------------------------------------------------------------------*/
void handle_post_control(http_server_t srv, http_request_t req,
                         void *user_data) {
...
int main(void) {
  ...
  if (cy_wcm_connect_ap(&connect_param, &ip_addr) == CY_RSLT_SUCCESS) {
    printf("Підключено! IP-адреса: %d.%d.%d.%d\n", (ip_addr.ip.v4 >> 24) & 0xFF,
           (ip_addr.ip.v4 >> 16) & 0xFF, (ip_addr.ip.v4 >> 8) & 0xFF,
           ip_addr.ip.v4 & 0xFF);
  } else {
  ...
  /* REST API */
  http_server_register_resource(server, "/api/status", HTTP_GET,
                                handle_get_status, NULL);
  http_server_register_resource(server, "/api/control", HTTP_POST,
                                handle_post_control, NULL);
